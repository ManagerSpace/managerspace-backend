                    ...
                    <form action="{{ route('invoices.update', $invoice) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="col-span-2">
                                ...
                                <textarea id="notes" name="notes" rows="4" class="block mt-1 w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm">{{ old('notes', $invoice->notes) }}</textarea>
                            </div>
                        </div>
                            ...
                            <div id="products-container">
                                @foreach($invoice->products as $index => $product)
                                    <div class="grid grid-cols-6 gap-4 mt-4">
                                        <div>
                                            <x-label for="products[{{ $index }}][category_id]" value="Category" />
                                            <select id="products[{{ $index }}][category_id]" name="products[{{ $index }}][category_id]" class="block mt-1 w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm" required>
                                        ...
                                        <div>
                                            <x-label for="products[{{ $index }}][name]" value="Product Name" />
                                            <x-input id="products[{{ $index }}][name]" class="block mt-1 w-full" type="text" name="products[{{ $index }}][name]" :value="$product->name" required />
                                        </div>
                                        <div>
                                            <x-label for="products[{{ $index }}][description]" value="Description" />
                                            <x-input id="products[{{ $index }}][description]" class="block mt-1 w-full" type="text" name="products[{{ $index }}][description]" :value="$product->description" />
                                        </div>
                                        <div>
                                            <x-label for="products[{{ $index }}][price]" value="Price" />
                                            <x-input id="products[{{ $index }}][price]" class="block mt-1 w-full" type="number" name="products[{{ $index }}][price]" step="0.01" :value="$product->price" required />
                                        </div>
                                        <div>
                                            <x-label for="products[{{ $index }}][tax_id]" value="Tax" />
                                            <select id="products[{{ $index }}][tax_id]" name="products[{{ $index }}][tax_id]" class="block mt-1 w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm" required>
                                                @foreach($taxes as $tax)
                                                    <option value="{{ $tax->id }}" {{ $product->tax_id == $tax->id ? 'selected' : '' }}>{{ $tax->name }} ({{ $tax->rate }}%)</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div>
                                            <x-label for="products[{{ $index }}][quantity]" value="Quantity" />
                                            <x-input id="products[{{ $index }}][quantity]" class="block mt-1 w-full" type="number" name="products[{{ $index }}][quantity]" :value="$product->quantity" required />
                                        </div>
